...
\]
La singola funzione $f_{\overline n}(x)$ è continua, per le ipotesi, in $x_0$, quindi in un intorno di tale punto $\exists\delta>0$ per cui
\[
\forall x\in(x_0-\delta,x_0+\delta)\ \abs{f_{\overline n}(x)-f_{\overline n}(x_0)}<\epsilon.
\]
Allora, in tale intorno, si ha
\[
\abs{f(x)-f(x_0)}<3\epsilon,
\]
quindi la funzione limite è continua.
\end{proof}
...
La derivabilità di una successione di funzioni è una questione più complicata dell'integrabilità: si è già visto in un precedente esempio che anche se la successione converge uniformemente, non è sempre vero che converge anche la successione delle derivate, come per $f_n(x)=\sqrt{x^2+\frac1{n^2}}$ la derivata della funzione limite della quale non esiste nemmeno in $x=0$.
...
\end{split}\]
e poiché per le ipotesi $f_n(x_0)\to f(x_0)$ per $n\to\pinf$, ed $\{f_n'\}$ converge a $g$, si ha che $\{f_n\}$ converge a $f$; la convergenza è infine uniforme in quanto non dipende affatto dalla scelta di $x$, come si nota nell'ultima riga dell'equazione ce la variabile non compare più.
\end{proof}
...
\]
in cui la norma si intende la norma euclidea in $\R^p$. È un'operazione binaria definita come $d\colon\boun{F}{E}\times\boun{F}{E}\to\R$.
