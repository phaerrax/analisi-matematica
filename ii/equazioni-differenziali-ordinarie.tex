...
Ora bisogna sistemare il problema in $x=0$. La soluzione deve essere innanzitutto cercata nell'intervallo in $(0,+\infty)$ se $x_0>0$, in $(-\infty,0)$ se $x_0<0$. Trovata la soluzione nel caso principale, essa sarà estendibile in $x=0$ se il prolungamento trovato (tramite l'altra soluzione nell'altro intervallo) è di classe $\cont{k}$. Una volta trovato, la funzione si estende naturalmente a $(-\infty,0)$ in cui è già definita e di classe $\cont{k}$, cioè sarà definita in tutto $\R$. Notare che il prolungamento non è necessariamente unico!
