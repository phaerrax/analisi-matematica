...
Come al solito, prendiamo la parametrizzazione $\vphi\colon D\subset\R^2\to\R^3$ di una superficie regolare $S$ immersa in $\R^3$, cioè tale per cui $\vphi(D)=S$: definiamo l'insieme $S_\circ\defeq\vphi(\mathring{D})$.\footnote{
	Nonostante la scrittura possa suggerirlo, non è vero che $S_\circ=\mathring{S}$: infatti $S$ è una superficie regolare in $\R^3$, quindi $\mathring{S}=\emptyset$.
}
Esso è tale che la restrizione della parametrizzazione $\vphi|_{\mathring{D}}\colon\mathring{D}\to S_\circ$ è biunivoca.
L'iniettività infatti discende dalla definizione di regolarità della superficie ($\vphi\colon D\to S$ è iniettiva) mentre la suriettività è ovvia per come è stata definita la funzione, poich\'e $S_\circ=\vphi(\mathring{D})=\vphi|_{\mathring{D}}(\mathring{D})$.\footnote{
	L'insieme $S_\circ$ può essere visto come l'interno della superficie, ma con una precisazione: certamente non è aperto come sottoinsieme di $\R^3$, poich\'e ogni intorno di un punto di $S_\circ$ contiene punti che non appartengono a $S_\circ$.
	È vero però che se $B$ è un intorno di un punto di $S_\circ$ allora $B\cap S_\circ$ è tutto contenuto in $S_\circ$, dunque $S_\circ$ è aperto \emph{nella topologia indotta su $S$} dalla usuale topologia di $\R^n$.
}
...
Per ogni $\vec x\in S_\circ$ possiamo associare un versore normale $\vnu(\vec x)$, definito come
\begin{equation}
	\vnu(\vec x)=\frac{\drp{\vphi}{u}(u,v)\times\drp{\vphi}{v}(u,v)}{\norm{\drp{\vphi}{u}(u,v)\times\drp{\vphi}{v}(u,v)}}
\end{equation}
con $(u,v)=\vphi^{-1}(\vec x)$: tale inversa è ben definita perch\'e $\vphi|_{\mathring{D}}$ è biiettiva.
...
La mappa $\vnu\colon S_\circ\to\R^3$ è continua, e dipende ``in modo continuo'' dall'inclinazione della superficie.
\begin{definizione} \label{d:superficie-orientabile}
	Sia $S\subset\R^3$ una superficie regolare parametrizzata da $\vphi\colon D\to\R^3$, e chiamiamo $S_\circ=\vphi(\mathring{D})$.
	La superficie $S$ si dice orientabile se il campo del versore normale $\vnu$ si può estendere con continuità da $S_\circ$ a tutta $S$.
\end{definizione}
Possiamo quindi anche trovare una \emph{riparametrizzazione} di una superficie: prese le parametrizzazioni $\vphi\colon D\to\R^3$ e $\vpsi\colon T\to\R^3$, esse sono equivalenti tramite la funzione $\vtau\colon D\to T$ se $S_\circ=\vphi(\mathring{D})=\vpsi(\mathring{T})$ e se, ovviamente, dato $\vec x\in S_\circ$ si ha $\vec x=\vphi(\vec y)=\vpsi\big(\vtau(\vec y)\big)$, per ogni $\vec y\in D$.
...
Per la continuità del versore normale $\vnu_{\vpsi}$, il segno della matrice jacobiana può essere quindi o 1 o $-1$ in tutto il dominio, perciò vale $\vnu_{\vpsi}(\vec x)=\pm\vnu_{\phi}(\vec x)$ per ogni $\vec x\in S_\circ$.
