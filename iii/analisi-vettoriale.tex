...
Prendiamo ora la parametrizzazione $\vphi\colon D\subset\R^2\to\R^3$ di una superficie regolare $S$ immersa in $\R^3$, cioè tale per cui $\vphi(D)=S$: definiamo l'insieme $S_\circ\defeq\vphi(\mathring{D})$.
...
Vediamo come varia il versore normale quando cambiamo la parametrizzazione: siano $\vphi=\vphi(u,v)\colon D\to S$ e $\tilde{\vphi}=\tilde{\vphi}(s,t)\colon\tilde{D}\to S$ due superfici regolari, e sia $\vpsi\colon\tilde{A}\to A$ un diffeomorfismo $(s,t)\mapsto(u,v)$ tra i due aperti $A\supset D$ e $\tilde{A}\supset\tilde{D}$.
Allora, detto $\vnu$ il versore normale a $S$ con la parametrizzazione $\vphi$ e $\tilde{\vnu}$ quello con $\tilde{\vphi}$, risulta
\begin{equation}
	\begin{split}
		\drp{\tilde{\vphi}}{s}\times\drp{\tilde{\vphi}}{t}&=\bigg(\drp{\vphi}{u}\drp{\psi_1}{s}+\drp{\vphi}{v}\drp{\psi_2}{s}\bigg)\times\bigg(\drp{\vphi}{u}\drp{\psi_1}{t}+\drp{\vphi}{v}\drp{\psi_2}{t}\bigg)=\\
		&=\drp{\vphi}{u}\drp{\psi_1}{s}\times\drp{\vphi}{v}\drp{\psi_2}{t}+\drp{\vphi}{v}\drp{\psi_2}{s}\times\drp{\vphi}{u}\drp{\psi_1}{t}=\\
		&=\drp{\psi_1}{s}\drp{\psi_2}{t}\bigg(\drp{\vphi}{u}\times\drp{\vphi}{v}\bigg)-\drp{\psi_2}{s}\drp{\psi_1}{t}\bigg(\drp{\vphi}{u}\times\drp{\vphi}{v}\bigg)=\\
		&=\det(\jac\vpsi)\bigg(\drp{\vphi}{u}\times\drp{\vphi}{v}\bigg)=\\
	\end{split}
\end{equation}
dunque normalizzando si ha
\begin{equation}
	\tilde{\vnu}=\sgn\big(\!\det(\jac\vpsi)\big)\vnu.
	\label{eq:versore-normale-riparametrizzazione}
\end{equation}
Per la continuità del versore normale, il segno della matrice jacobiana può essere quindi o 1 o $-1$ in tutto il dominio, perciò vale $\tilde{\vnu}(\vec x)=\pm\vnu(\vec x)$ per ogni $\vec x\in S_\circ$.
...
Una superficie non orientabile è il nastro di M\"obius, dato dalla parametrizzazione
\begin{equation}
	\vphi(u,v)=\bigg(
