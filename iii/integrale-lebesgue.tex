...
Per prima cosa, l'insieme vuoto avrà certamente una misura (che sarà chiaramente zero), dunque $\mathcal C$ lo dovrà includere.
...
La collezione $\mathcal C$ di insiemi su cui definire $\lambda$ dovrà essere di questo tipo.
Possiamo dare quindi una definizione su cui basare il concetto di volume generalizzato, detto volgarmente \emph{misura}.
...
Per le unioni o intersezioni infinite (numerabili) non vale più il teorema dimostrato, ma dobbiamo dimostrarlo nuovamente: dimostriamo finalmente, quindi, la proprietà di additività numerabile che abbiamo annunciato all'inizio di questa sezione.
...
Indicheremo con $\vec z=(\vec x,\vec y)\in\R^m\times\R^k$ il punto $\vec z=(x_1,\dots,x_m,y_1,\dots,y_k)\in\R^{m+k}$, e per brevità abbrevieremo
\begin{equation*}
	\int f(\vec z)\,\dd\vec z=\int f(\vec z)\,\dd z_1\,\dd z_2\dots\dd z_n.
\end{equation*}
...
\begin{equation*}
	\int_{\R^n}f(\vec z)\,\dd\vec z=\int_\R\bigg[\int_\R\bigg[\dots\int_\R\bigg[\int_\R f(\vec z)\,\dd z_1\bigg]\dd z_2\bigg]\dots\dd z_{n-1}\bigg]\dd z_n
\end{equation*}
e non importa l'ordine in cui lo si scompone.
...
\end{teorema}
\begin{proof}
	Innanzitutto, per il segno costante di $f$, l'integrale $\int_{\R^n}f\,\dd\mu$ esiste certamente, poich\'e la parte negativa è identicamente nulla.
	Per il resto, ci può riportare al teorema \ref{t:fubini} di Fubini: le funzioni $\vec x\mapsto f(\vec x,\vec y)$ e $\vec y\mapsto f(\vec x,\vec y)$ sono misurabili quasi ovunque in $\R^m$ e $\R^k$ rispettivamente, cos\'i come lo sono le
	\begin{equation*}
		\vec x\mapsto\int_{\R^k}f(\vec x,\vec y)\,\dd\vec y\qqq\vec y\mapsto\int_{\R^m}f(\vec x,\vec y)\,\dd\vec x
	\end{equation*}
	\dots
\end{proof}
...
\end{equation}
dove $\rho(\vec x)$ è il resto di Peano per il quale $\rho(\vec x)=o(\norm{\vec x-\vec x_0})$, che tende a zero per $\vec x\to\vec x_0$.
...
\begin{teorema} \label{t:composizione-diffeomorfismo-misurabile}
	...
	\begin{itemize}
		...
		\item se $f\geq 0$ in $V$, allora
			\begin{equation}
				\int_Bf(\vec y)\,\dd\vec y<+\infty\text{ se e solo se }\int_A(f\circ\vphi)(\vec x)\abs{\det\jac\vphi(\vec x)}\dd\vec x.
			\end{equation}
	\end{itemize}
\end{teorema}
Se prendiamo la funzione caratteristica di $B$, in particolare, troviamo che
\begin{equation}
	\int_B\chi_B(\vec y)\,\dd\vec y=\int_B\dd\vec y=\int_A(\chi_B\circ\vphi)(\vec x)\abs{\det\jac\vphi(\vec x)}\dd\vec x=\int_A\abs{\det\jac\vphi(\vec x)}\dd\vec x
\end{equation}
ossia il determinante della jacobiana ci dice (nell'approssimazione lineare) come si trasforma la misura cambiando le coordinate: in particolare se la trasformazione è lineare la jacobiana sarà costante, di conseguenza avremmo $\mu(B)=\mu\big(\vphi(A)\big)=\abs{\det\jac\vphi}\mu(A)$.
Sfruttando lo sviluppo in serie al primo ordine, il fattore $\abs{\det\jac\vphi}$ indica la trasformazione solo a livello infinitesimo: possiamo vedere $\dd\vec y$ ad esempio come un ``elemento infinitesimo di volume'' (o di area), e il risultante $\abs{\det\jac\vphi(\vec x)}\dd\vec x$ come l'elemento di volume nelle nuove coordinate.

Prendiamo il celebre esempio delle coordinate polari in $\R^2$: possiamo passare dalle coordinate cartesiane ortogonali a queste con la trasformazione
\begin{equation}
	\begin{pmatrix}
		x\\y
	\end{pmatrix}
